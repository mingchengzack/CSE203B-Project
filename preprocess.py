...
def main():
    ...
    cats=['talk.politics.misc','sci.electronics','sci.space','rec.sport.baseball','comp.graphics','comp.os.ms-windows.misc','talk.religion.misc','comp.sys.ibm.pc.hardware',
          'comp.sys.mac.hardware','comp.windows.x','misc.forsale','rec.autos','rec.motorcycles','rec.sport.hockey','sci.crypt','sci.med','alt.atheism',
          'soc.religion.christian','talk.politics.guns','talk.politics.mideast']
    for i in [5, 10, 15, 20]:
        newsgroups_train = fetch_20newsgroups(data_home="./data/", subset='all', remove=('headers', 'footers', 'quotes'),categories=cats[:i])
        # newsgroups_test = fetch_20newsgroups(data_home="./data/", subset='test', remove=('headers', 'footers', 'quotes'),categories=cats[:i])
        ...
        torch.save(x_train_tensor,'./data/x_train' + '_' + str(i) + '.pt')
        # torch.save(x_test_tensor,'./data/x_test' + '_' + str(i) + '.pt')
        torch.save(y_train_tensor,'./data/y_train' + '_' + str(i) + '.pt')
        # torch.save(y_test_tensor,'./data/y_test' + '_' + str(i) + '.pt')
